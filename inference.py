...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        if len(self.keys()) == 0 or self.total() == 0:
            return
        divider = self.total()
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPotentialPosition in self.allPositions:
            probability = self.getObservationProb(observation, pacmanPosition, ghostPotentialPosition, jailPosition)
            self.beliefs[ghostPotentialPosition] *= probability
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPosition in self.allPositions:
            newGhostPositionDist = self.getPositionDistribution(gameState, oldPosition)
            for newPosition in self.allPositions:
                newBeliefs[newPosition] += self.beliefs[oldPosition] * newGhostPositionDist[newPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position, numberParticle in self.particles:
             beliefs[position] *= self.getObservationProb(observation, pacmanPosition, position, jailPosition)
        beliefs.normalize()

        samples = [beliefs.sample() for _ in range(self.numParticles)]
